...
export async function startConsumer(){
    ...
    console.log(`Kafka consumer connectd and subscribed to "${TOPIC}".`)
    ...
    await consumer.run({
        eachMessage: async ({message}: EachMessagePayload) => {
            ...
            try {
                ...
                //if this is the first message in an empty array then start the start the timer
                if(buffer.length === 1 && !flushTimer){
                    flushTimer = setTimeout(flushBufferToDb, BATCH_INTERNAL_MS)
                }
            } catch (error) {
