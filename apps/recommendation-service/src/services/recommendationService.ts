...
export const recommendProducts = async(userId:string,allProducts:any):Promise<string[]> => {
    ...
    const {interactions} = processData as {
    ...
    // Faltten the 2D embeddings into 1D feature vectors
    ...
    const output = tf.layers
     ...
     // convert user and product interactions into tensors for traning
     ...
     const predictions = model.predict([
        tf.tensor2d([[userMap[userId]  ?? 0]], [1,1], "int32"),
        productTensorAll,
     ]) as tf.Tensor
     ...
     const recommendProducts: RecommendedProduct[] = Object.keys(productMap).map(
        (productId, index) => ({
            productId,
